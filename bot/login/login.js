// set bash title
process.stdout.write("\x1b]2;Goat Bot V2 - Made by NTKhang modified by Developer Rasin\x1b\x5c");
...
let subTitle = `THIS BOT MODIFIED BY TASBIUL ISLAM RASIN 😻`;
const subTitleArray = [];
...
async function startBot(loginWithEmail) {
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
			...
			logColor("#f5ab00", createLine("[DEVELOPER RASIN] BOT INFO"));
			log.info("[DEVELOPER RASIN] NODE VERSION", process.version);
			log.info("[DEVELOPER RASIN] PROJECT VERSION", currentVersion);
			log.info("[DEVELOPER RASIN] BOT ID", `${global.botID} - ${await getName(global.botID)}`);
			log.info("[DEVELOPER RASIN] PREFIX", global.GoatBot.config.prefix);
			log.info("[DEVELOPER RASIN] LANGUAGE", global.GoatBot.config.language);
			log.info("[DEVELOPER RASIN] BOT NICK NAME", global.GoatBot.config.nickNameBot || "GOAT BOT");
			// ———————————————————— GBAN ————————————————————— //
			let dataGban;
			...
			// —————————————————— COPYRIGHT INFO —————————————————— //
			// console.log(`\x1b[1m\x1b[33mCOPYRIGHT:\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36mThis bot modified by Developer Rasin. Thanks for using my bot 😍\x1b[0m`);
			console.log(`\x1b[1m\x1b[33m${("COPYRIGHT:")}\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36m${("This bot modified by Developer Rasin. Thanks for using my bot 😍")}\x1b[0m`);
			logColor("#f5ab00", character);
